...
use lopdf::{
    dictionary, Bookmark, Document, EncryptionState, EncryptionVersion, Object, ObjectId,
    Permissions, Stream,
};
...
fn main() -> AnyResult<()> {
    ...
    verify_bundle(&hearing, &full_entries, FULL_EXPECTED_PAGES, SOURCE_DOCUMENTS, 15)?;
...
fn generate_source_document(doc_no: usize, title: &str, landscape_last_page: bool) -> AnyResult<Document> {
...
fn generate_index_document(entries: &[BundleEntry], total_pages: usize) -> AnyResult<Document> {
    ...
    let mut ops = vec![
        ...
        Operation::new("Tj", vec![Object::string_literal("SYNTHETIC COURT BUNDLE INDEX")]),
    ...
    for entry in entries {
        ops.push(Operation::new("Td", vec![0.into(), (-INDEX_ROW_DROP).into()]));
    ...
    let stream_id = doc.add_object(Stream::new(dictionary! {}, Content { operations: ops }.encode()?));
    let page_id = doc.add_object(dictionary! {
        "Type" => "Page",
        "Parent" => pages_id,
        "Contents" => stream_id,
...
fn validate_source(path: &Path) -> AnyResult<Document> {
    ...
    for page_no in pages.keys().copied() {
        ...
        if text.trim().is_empty() {
            return Err(format!("source page is not searchable: {} page {page_no}", path.display()).into());
        }
    }
    Ok(doc)
}

fn build_bundle(specs: &[SourceSpec], output: &Path, expected_pages: usize) -> AnyResult<Vec<BundleEntry>> {
    ...
    if computed_pages != expected_pages {
        return Err(format!("bundle page plan mismatch: computed={computed_pages} expected={expected_pages}").into());
    }
...
fn verify_bundle(
    ...
    if pages.len() != expected_pages {
        return Err(format!("assembled page mismatch: {} != {expected_pages}", pages.len()).into());
    }
    ...
    let expected_landscape = if entries.iter().any(|e| e.title.ends_with("12")) { 1 } else { 0 };
    if landscape_pages != expected_landscape {
        return Err(format!("orientation inventory mismatch: {landscape_pages} != {expected_landscape}").into());
    }
    ...
    if outline_count != expected_bookmarks {
        return Err(format!("bookmark count mismatch: {outline_count} != {expected_bookmarks}").into());
    }
    Ok(())
}
...
fn verify_index_links(doc: &Document, entries: &[BundleEntry], expected_links: usize) -> AnyResult<()> {
    let pages = doc.get_pages();
    ...
    if annots.len() != expected_links || annots.len() != entries.len() {
        return Err(format!("index link count mismatch: {} != {expected_links}", annots.len()).into());
    }
    for (annot, entry) in annots.iter().zip(entries) {
        ...
        let target_id = dest.first().ok_or("empty link destination")?.as_reference()?;
...
fn write_receipt(
    ...
    let source_rows = specs
        .iter()
        .map(|s| format!("    {{\"section\":{},\"file\":\"{}\"}}", s.section, s.path.file_name().unwrap().to_string_lossy()))
        .collect::<Vec<_>>()
        .join(",\n");
    let json = format!(
...
mod tests {
    ...
    fn full_bundle_plan_is_over_one_hundred_pages() {
        ...
        assert!(FULL_EXPECTED_PAGES > 100);
    }
    ...
    fn core_bundle_plan_is_small_and_issue_led() {
        ...
        assert!(CORE_EXPECTED_PAGES < FULL_EXPECTED_PAGES);
    }
}
