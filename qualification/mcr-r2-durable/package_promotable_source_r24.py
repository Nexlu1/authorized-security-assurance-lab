...
out = Path(os.environ["RUNNER_TEMP"]) / "mcr-r2-promotable-source-r24"
if out.exists():
    shutil.rmtree(out)
...
for name in (
    ...
    "R2_4_RESOURCE_LIMIT_EXPANSION_RECEIPT.json",
...
fixture_dir = Path(os.environ.get("MCR_R24_FIXTURE_DIR", Path(os.environ["RUNNER_TEMP"]) / "mcr-r24-resource-fixtures"))
fixture_manifest = fixture_dir / "R24_RESOURCE_FIXTURE_MANIFEST.json"
if fixture_manifest.is_file():
    shutil.copy2(fixture_manifest, control / fixture_manifest.name)
...
manifest = {
    "schema": "mcr-r2-4-promotable-source-v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "github_sha": os.environ.get("GITHUB_SHA"),
    "source_zip_sha256": "a029571403ee89d42848b90e419384e9d9138534b43733b81483ee2d296d7215",
    "status": "PROMOTABLE_SOURCE_AFTER_57_OF_57_ENGINEERING_FUNCTIONAL_GATE_TARGET_RESOURCE_CALIBRATION_PENDING",
    "matrix_functional_coverage": "57/57",
    "resource_cases": ["ARC-008", "ARC-009", "ARC-011"],
    "resource_semantics": "hard accepted-byte stream limits independent of untrusted ZIP uncompressed-size metadata; no filesystem materialisation",
    "production_numeric_limits": "PENDING_TARGET_WINDOWS_PRESSURE_CALIBRATION",
    "file_count": len(rows),
    "files": rows,
